...
// One downloadable .vxm per app mode. A ModelSpec is the single place a model's HuggingFace location,
// size, and pinned digest live — repointing a model to a new upload means editing the one entry below.
data class ModelSpec(
    val id: String,             // stable key: state-map entry, never shown to the user
    val displayName: String,
    val mode: String,           // the app mode this model powers (library card subtitle)
...
object ModelCatalog {
    ...
    val ALL = listOf(QWEN, SMOLVLM2, DL3DV)
}
