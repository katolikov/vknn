...
An Android app with three modes over the VKNN engine — **Chat** (Qwen2.5-Coder-0.5B or Llama-3.2-1B / Llama-3.1-8B Instruct), **VLM**
...
lists the Qwen and Llama-3 variants, VLM the SmolVLM2 variants (fp16 and int4). The choice persists and
...
app-demo/
  ...
  app/                    the Android app (Kotlin + Compose)
    src/main/java/com/vknn/chat/
      ...
      Tokenizer.kt          pure-Kotlin byte-level BPE (Qwen2, SmolVLM2, and Llama-3 pipelines)
      model/                model library: catalogue (fp16/int4 variants; built-ins merged with a
                            remote catalog.json manifest, so new models ship without an APK rebuild)
                            + resumable sha256-verified HF downloads, per-tab variant selection,
                            backend + prompt settings, CPU-admission policy
    ...
    src/main/assets/        vocab.json + merges.txt (the Qwen tokenizer ships in the APK; the
                            SmolVLM2 and Llama-3 tokenizers download with their models)
    src/test/               JVM tests: all three BPE pipelines match HuggingFace; HF API parsing;
...
**Chat** — a with-past decoder (Qwen2 or Llama-3, chosen in the variant picker) compiled at a fixed context length (**C = 1024** for the Qwen
...
Prompts are wrapped in the model's chat-template dialect — ChatML for the Qwen variants, the
Llama-3 header format for the Llama variants (`ChatPromptTemplate`, selected by the catalogue's
`chatDialectId`); a per-dialect template override is editable in the app's prompt editor.

**VLM** — SmolVLM2-2.2B as **one multi-graph `.vxm`** (a vision encoder plus text-prefill,
image-prefill, and decode decoder buckets over one shared weight pool — the token-embedding lookup
is fused into the decoders, which take `input_ids` directly; see the repo's `docs/running-a-vlm.md`),
...
asks a question about the current frame — editable in the prompt editor, with a live token-budget
check against the model's prefill window — and shows the streamed answer.
...
int4), Llama-3.2-1B 1.2 GB int4, Llama-3.1-8B 5.3 GB int4, SmolVLM2 4.5 GB fp16 (1.35 GB as int4),
YoNoSplat encoder 2.3 GB. Running fp16 SmolVLM2 on Vulkan needs **~4.6 GB of
GPU-addressable memory** (the int4 variant roughly a third of that); Llama-3.1-8B peaks at ~6.4 GB.
