#!/usr/bin/env bash
# Push vxrt binaries + model + assets to the device, run the classify example, pull results.
...
[[ -f "$BUILD_DIR/vx_classify" ]] || { log "building"; ./scripts/build_android.sh; }
...
adb push "$BUILD_DIR/vx_classify" "$BUILD_DIR/vx_probe" "$BUILD_DIR/vx_profile" \
         "$BUILD_DIR/vx_backend_switch" "$BUILD_DIR/vx_ion_zerocopy" "$DEV_DIR/" >/dev/null
