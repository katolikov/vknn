...
#   image + intrinsics -> encoder (vxrt Vulkan) -> Gaussians -> Vulkan rasterizer -> rendered view (PPM).
# Prereqs on device (/data/local/tmp/vxrt/yono): encoder_fp16.vxm, yono_image.bin, yono_intrinsics.bin.
# Build + push first:  ./scripts/build_android.sh && adb push build-android/vx_yonosplat /data/local/tmp/vxrt/
set -e
DIR=/data/local/tmp/vxrt
adb push build-android/vx_yonosplat $DIR/vx_yonosplat >/dev/null
adb shell chmod +x $DIR/vx_yonosplat
...
adb shell "cd $DIR/yono && rm -rf dump && mkdir -p dump && \
  VXRT_DUMP_NAMES=/enc/Reshape_7_output_0 ../vx_run_io encoder_fp16.vxm out --no-weight-cache \
  yono_image.bin yono_intrinsics.bin >/dev/null 2>&1 || true"

echo '== full pipeline: image -> encoder(GPU) -> Gaussians -> rasterizer(GPU) -> view =='
adb shell "cd $DIR/yono && ../vx_yonosplat encoder_fp16.vxm yono_image.bin yono_intrinsics.bin \
