#!/usr/bin/env bash
# Benchmark vxrt on-device across backends/precisions; report latency/fps + cold/warm session.
set -euo pipefail
...
[[ -f "$BUILD_DIR/vx_classify" ]] || ./scripts/build_android.sh
adb shell "mkdir -p $DEV_DIR"
adb push "$BUILD_DIR/vx_classify" assets/mobilenetv2.onnx assets/input.bin assets/golden.bin "$DEV_DIR/" >/dev/null
adb shell "chmod +x $DEV_DIR/vx_classify"

run() { # backend precision label
  adb shell "cd $DEV_DIR && ./vx_classify --model mobilenetv2.onnx --input input.bin \
...
adb shell "cd $DEV_DIR && rm -rf cache && ./vx_classify --model mobilenetv2.onnx --input input.bin \
