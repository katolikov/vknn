...
| `flatLayout` | bool | `true` / `false` | `true` | Flat row-major GPU layout pass that keeps generic head ops (Transpose/Slice/Concat/Binary/Softmax) on the GPU. On by default (fastest). `false` (CLI `--no-flat`) skips the pass only on a graph whose every op has an NC4HW4 kernel; a graph containing an op whose descriptor layout is Flat or ShapeDependent (Transpose, Slice, Gather, Binary/Add, ArgMax, Mod, ...) keeps the pass on, since those kernels have no NC4HW4 plan — advanced. Backed by `Hint::FlatLayout`. |
