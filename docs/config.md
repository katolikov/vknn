...
| `flatLayout` | bool | `true` / `false` | `true` | Flat row-major GPU layout pass that keeps generic head ops (Transpose/Slice/Concat/Binary/Softmax) on the GPU. On by default (fastest). `false` (CLI `--no-flat`) skips the pass only on a graph of ops whose descriptor layout class is Nc4 (Conv, Relu, the pools, Resize, ...). Any op whose descriptor layout is Flat or ShapeDependent keeps the pass on: the Flat ops (Transpose, Slice, MatMul, Clip, Reduce, Where, ArgMax, Mod, ...) have no NC4HW4 plan, and the rule reads the class, not the node, so the ShapeDependent ops keep it on too, including Add, Binary, Concat, Split, Softmax and ConvTranspose nodes that run NC4HW4 kernels. On most real graphs (any Add or Clip) `--no-flat` therefore changes nothing — advanced. Backed by `Hint::FlatLayout`. |
