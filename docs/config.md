...
| `--quant-bits 4\|8` | Packed-weight bit width for `-Os` (default `4`). `8` trades half the weight-traffic saving for ~an order of magnitude lower quantization error, through the same outlier/min-MSE/bias-correction/guard pipeline and its own native GPU MatMul kernels. |
