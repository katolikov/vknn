...
Goal: figure out *why* MNN's Vulkan path beats VKNN on the same device, and decide what to
change. Based on reading MNN's source (`source/backend/vulkan/`) plus experiments on-device.
...
1. **Two Vulkan backends, and the default is image-based.**
   ...
   So a stock `MNN_VULKAN=ON` build (the one we benchmarked) uses the **image** backend, storing
...
2. **`c8w4` register tiling for 1×1 conv** (`convolution1x1_c8w4.comp`): each thread computes
   **8 output channels (2 blocks) × 4 width** as an *outer-product* microkernel
   ...
   each weight texel feeds 4 width positions. That's 8 `vec4` accumulators per thread.
...
SSBO those same reads pressure the L1/L2, and worse, the heavy tiling needs more registers, which
drops occupancy.
...
texture-cached reads. In SSBO we hit the register/occupancy wall first.** So the highest-
...
Before the full port, `vknn_image_bench` measures image-backed c8w4 1×1 conv against the SSBO
version on representative shapes — so a number justifies the rewrite, not a hunch.
...
beat SSBO, and converting VKNN to images would make it *slower*.

**Revised conclusion: MNN's advantage isn't the storage type — it's kernel quality plus per-shape
tuning.** MNN wins *even while paying the image penalty on this GPU*, so its conv
microkernels (c8w4, gemm16x16, tuned Winograd) and op selection are just better optimized than
ours. The right direction for VKNN is better **SSBO** kernels and **less inter-op
