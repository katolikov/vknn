...
## Decision
Three disk caches live under `config.cacheDir`, keyed implicitly by device+driver via the pipeline
...
- Caches are safe to delete and regenerate. The pipeline cache is the dominant warm-start win;
