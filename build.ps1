...
if ($clean -and -not ($android -or $convertOnly -or $docs -or $test)) {
    ...
    Remove-Item -Recurse -Force -ErrorAction Ignore build-host, build-android
    exit 0
}
...
# build-host\Release\ for multi-config (Visual Studio).
function HostBinary([string]$BuildDir, [string]$Name) {
    foreach ($p in @("$BuildDir/$Name.exe", "$BuildDir/Release/$Name.exe", "$BuildDir/$Name")) {
...
if ($test) {
    $buildDir = "build-host"
    if ($clean) { Write-Host ">> clean: removing $buildDir"; Remove-Item -Recurse -Force -ErrorAction Ignore $buildDir }
