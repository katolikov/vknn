...
everything (the model + the sample inputs/goldens the example config uses) into one folder.
  ...
  python benchmark/scripts/fetch_model.py [--repo katolikov/yonosplat-vknn] [--out benchmark/models] [--files ...]
...
Then run:  python benchmark/run.py run benchmark/configs/yonosplat.json
...
print(f"\ndone -> {args.out}.  run:  python benchmark/run.py run benchmark/configs/yonosplat.json")
