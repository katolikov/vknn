...
//   image + intrinsics --> encoder (vxrt Vulkan) --> Gaussians --> Vulkan rasterizer --> rendered view.
// The encoder runs as a normal vxrt Session; its 6 Gaussian outputs feed the from-scratch Vulkan
// compute rasterizer (preprocess -> GPU tile-bin -> bitonic sort -> per-tile alpha compositing). The
// rendered view is written as a PPM. See scripts/yonosplat/ for how the encoder .vxm + inputs are made.
//
//   vx_yonosplat <encoder.vxm> <image.bin> <intrinsics.bin> <out.ppm> [--extr extr.bin] [--view N]
// image.bin = fp32 [1,V,3,224,224], intrinsics.bin = fp32 [1,V,3,3] (normalized). extr.bin (optional) =
// fp32 [V,4,4] camera-to-world (the encoder's predicted pose, dumpable via VXRT_DUMP_NAMES); identity
// if omitted. Renders view N.
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
int main(int argc, char** argv) {
...
#else
  if (argc < 5) {
    printf("usage: vx_yonosplat <encoder.vxm> <image.bin> <intrinsics.bin> <out.ppm> [--extr extr.bin] [--view N]\n");
    return 1;
  }
  ...
  if (!sess) { fprintf(stderr, "failed to load encoder %s\n", enc.c_str()); return 1; }
  ...
  if (sess->run(ins, outs) != Status::kOk) { fprintf(stderr, "encoder run failed\n"); return 2; }
  auto get = [&](const char* nm) -> const float* {
    for (auto& o : outs) if (o.name == nm) return o.f32();
    fprintf(stderr, "encoder output '%s' missing\n", nm);
    return nullptr;
  };
  const float *means = get("means"), *covs = get("covariances"), *harm = get("harmonics"),
              *opac = get("opacities");
  if (!means || !covs || !harm || !opac) return 2;
  int N = 0;
  for (auto& o : outs) if (o.name == "means") N = (int)(numElements(o.shape) / 3);
  ...
  { auto d = readFile(intp); memcpy(intr.data(), d.data(), std::min(d.size(), intr.size() * 4)); }
  std::vector<float> extr;
  if (!extp.empty()) { auto d = readFile(extp); extr.assign((size_t)(view + 1) * 16, 0);
  ...
  float c2w[16];
  if (!extr.empty()) memcpy(c2w, &extr[(size_t)view * 16], 64);
  else { memset(c2w, 0, 64); c2w[0] = c2w[5] = c2w[10] = c2w[15] = 1.f; }  // identity
  const float* K = &intr[(size_t)view * 9];
  float Rt[9], tp[3];
  for (int r = 0; r < 3; ++r) for (int c = 0; c < 3; ++c) Rt[r*3+c] = c2w[c*4 + r];
  ...
  if (!ctx.initialized()) { fprintf(stderr, "no Vulkan for rasterizer\n"); return 1; }
  ...
  mB.upload(mv.data(), mv.size()*4); cvB.upload(cvv.data(), cvv.size()*4);
  clB.upload(cols.data(), cols.size()*4); opB.upload(opv.data(), opv.size()*4);
  ...
  int DB=32-tileBits;
  int64_t CAP=1; while(CAP<(int64_t)N*8 || CAP<(1<<16)) CAP<<=1;
  ...
  struct DupPC{int32_t N,ntx,nty,DB,CAP; float near,invRange;} dpc{N,ntx,nty,DB,(int)CAP,NEAR,invRange};
  ...
  for (uint32_t k=2;k<=(uint32_t)CAP;k<<=1)
    for (uint32_t j=k>>1;j>0;j>>=1){ BitPC bp{j,k,(uint32_t)CAP};
      bit.dispatch(cmd,{keyB.handle(),valB.handle()},&bp,sizeof(bp),(uint32_t)((CAP+255)/256)); vk::computeBarrier(cmd); }
  RngPC rc{0,nTiles,(int)CAP,DB}, ra{1,nTiles,(int)CAP,DB};
  rng.dispatch(cmd,{keyB.handle(),rgB.handle()},&rc,sizeof(rc),(uint32_t)((nTiles+255)/256)); vk::computeBarrier(cmd);
  rng.dispatch(cmd,{keyB.handle(),rgB.handle()},&ra,sizeof(ra),(uint32_t)((CAP+255)/256)); vk::computeBarrier(cmd);
  ...
  printf("[done] image -> %s  (mean=%.4f)\n", outp.c_str(),
         [&]{ double s=0; for (float x:img) s+=x; return s/img.size(); }());
  return 0;
#endif
}
