...
int main() {
    ...
    printf("driverID       : %u\n", c.driverID);
    printf("apiVersion     : %u.%u.%u\n", VK_VERSION_MAJOR(c.apiVersion), VK_VERSION_MINOR(c.apiVersion), VK_VERSION_PATCH(c.apiVersion));
    printf("vendor/device  : 0x%x / 0x%x\n", c.vendorID, c.deviceID);
    printf("subgroupSize   : %u (arith=%d shuffle=%d)\n", c.subgroupSize, c.subgroupArithmetic, c.subgroupShuffle);
    printf("maxWGInvoc     : %u\n", c.maxWorkGroupInvocations);
    printf("maxWGSize      : [%u,%u,%u]\n", c.maxWorkGroupSize[0], c.maxWorkGroupSize[1], c.maxWorkGroupSize[2]);
    ...
    for (uint32_t i = 0; i < mp.memoryTypeCount; ++i)
    {
        ...
        printf("  type %2u heap=%u flags=0x%03x %s%s%s%s%s\n", i, mp.memoryTypes[i].heapIndex, f, (f & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) ? "DEVICE_LOCAL " : "", (f & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) ? "HOST_VISIBLE " : "", (f & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) ? "HOST_COHERENT " : "", (f & VK_MEMORY_PROPERTY_HOST_CACHED_BIT) ? "HOST_CACHED " : "", (f & VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT) ? "LAZY " : "");
    }
    for (uint32_t i = 0; i < mp.memoryHeapCount; ++i)
    {
        printf("  heap %2u size=%.2f GiB flags=0x%x\n", i, mp.memoryHeaps[i].size / (1024.0 * 1024 * 1024), mp.memoryHeaps[i].flags);
    }
    printf("==== probe OK ====\n");
    return 0;
#else
