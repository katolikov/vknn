...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "device=%s subgroup=%u tsPeriod=%.3fns tsSupported=%d maxWGCount0=%u\n",
            caps.deviceName.c_str(), caps.subgroupSize, period, caps.timestampSupported,
    ...
    while (std::getline(mf, line)) {
        ...
        Run r{};
        if (!(is >> r.label >> r.spv >> r.N >> r.Cin >> r.H >> r.W >> r.Cout >> r.KH >> r.KW >> r.SH >>
              r.SW >> r.PT >> r.PL >> r.DH >> r.DW >> r.OCB >> r.LSX >> r.iters >> r.warmup >>
              r.verify))
            continue;
            ...
            for (int icb = 0; icb < Cinb; ++icb)
                for (int t = 0; t < r.KH * r.KW; ++t)
                    for (int l = 0; l < 4; ++l) {
                        ...
                        hwt[vec * 4 + l] = f2h((oc < r.Cout && ic < r.Cin)
                                                   ? synth((uint32_t)(vec * 5 + l * 131 + 1))
                                                   : 0.0f);
                    }
        ...
        for (int i = 0; i < 4; ++i)
            binds[i] = {(uint32_t) i, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_COMPUTE_BIT,
                        nullptr};
        ...
        PC pc{r.N,  r.Cin, r.H,  r.W,  r.Cout, OH,   OW,   r.KH,   r.KW,
              r.SH, r.SW,  r.PT, r.PL, r.DH,   r.DW, 0,    0.0f,   0.0f};
                        ...
                        for (int ox = 0; ox < OW; ++ox) {
                            ...
                            for (int ic = 0; ic < r.Cin; ++ic)
                                for (int ky = 0; ky < r.KH; ++ky) {
                                    ...
                                    if (iy < 0 || iy >= r.H) continue;
                                    for (int kx = 0; kx < r.KW; ++kx) {
                                        int ix = ox * r.SW - r.PL + kx * r.DW;
                                        if (ix < 0 || ix >= r.W) continue;
                                        int icb = ic / 4, il = ic % 4;
                                        size_t sidx =
                                            (((size_t)(n * Cinb + icb)) * r.H * r.W + iy * r.W + ix) * 4 + il;
                                        size_t vec = (((size_t) oc * Cinb + icb) * r.KH * r.KW +
                                                      (ky * r.KW + kx));
                                        acc += h2f(hsrc[sidx]) * h2f(hwt[vec * 4 + il]);
                                    }
                                }
                            size_t didx =
                                (((size_t)(n * Coutb + ocb)) * OH * OW + oy * OW + ox) * 4 + l;
                            float got = h2f(hdst[didx]);
                            maxerr = std::max(maxerr, (double) std::fabs(got - acc));
                        }
        }

        printf("%s,%s,%d,%d,%d,%d,%.5f,%.5f,%.1f,%.4g\n", r.label.c_str(), caps.deviceName.c_str(),
               r.OCB, r.LSX, OH, OW, msMin, msMed, gflops, maxerr);
        fflush(stdout);
