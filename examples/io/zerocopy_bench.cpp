...
// both loops with the same model and reports min / median wall per run, which is the number an
...
int main(int argc, char **argv) {
    if (argc < 2)
    {
        printf("usage: %s model.vxm [--repeat N] [--precision low|normal|high] [--power normal|high]\n", argv[0]);
        return 1;
    }
