...
static bool runAdd(vk::VulkanContext &ctx, vk::CommandRunner &runner, vk::ComputePipeline &add, VkBuffer a, VkBuffer b, VkBuffer c, uint32_t n) {
    uint32_t count = n;
...
int main() {
    ...
    if (ionA && ionA->data())
    {
        ...
        {
            printf("  dma-buf import into Vulkan unsupported on this driver -> would fall back to "
                   "staging\n");
        }
    } else
    {
