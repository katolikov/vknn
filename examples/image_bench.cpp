...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < Cin * H * W; ++i) in[i] = ((i * 37) % 23 - 11) * 0.03f;
  for (int i = 0; i < Cout * Cin; ++i) wt[i] = ((i * 13) % 17 - 8) * 0.02f;
  for (int oc = 0; oc < Cout; ++oc)
    for (int p = 0; p < H * W; ++p) {
      float acc = 0;
      for (int ic = 0; ic < Cin; ++ic) acc += wt[oc * Cin + ic] * in[ic * H * W + p];
      ref[oc * H * W + p] = acc;
    }
  auto cosErr = [&](const std::vector<float>& v) {
    double dot = 0, na = 0, nb = 0;
    for (size_t i = 0; i < ref.size(); ++i) { dot += v[i] * ref[i]; na += v[i] * v[i]; nb += ref[i] * ref[i]; }
  ...
  std::vector<float> ssboOut(Cout * H * W);
  {
    ...
    vk::Buffer inB(ctx, (size_t)Cinb * H * W * 4 * 2), outB(ctx, (size_t)Coutb * H * W * 4 * 2, vk::MemPref::kReadback);
    vk::Buffer wB(ctx, (size_t)Cout * Cinb * 4 * 2), bB(ctx, (size_t)Coutb * 4 * 2);
    { std::vector<fp16_t> t(Cinb * H * W * 4); pack(in.data(), t.data(), Cin, H * W); inB.upload(t.data(), t.size() * 2); }
    ...
    { std::vector<fp16_t> z(Coutb * 4, 0); bB.upload(z.data(), z.size() * 2); }

    vk::ComputePipeline pipe(ctx, "conv1x1", 4, sizeof(int) * 18);
    struct PC { int N, Cin, H, W, Cout, OH, OW, KH, KW, SH, SW, PT, PL, DH, DW, act; float lo, hi; }
    ...
    std::vector<fp16_t> t(Coutb * H * W * 4); outB.download(t.data(), t.size() * 2);
  ...
  if (vk::Image::supported(ctx)) {
    ...
    inI.toGeneral(runner); kI.toGeneral(runner); outI.toGeneral(runner);
    { std::vector<fp16_t> t((size_t)W * Cinb * H * 4, 0);  // input image [w+icb*W, h]
      for (int icb = 0; icb < Cinb; ++icb) for (int h = 0; h < H; ++h) for (int w = 0; w < W; ++w)
        for (int l = 0; l < 4; ++l) { int c = icb * 4 + l;
          t[(h * (W * Cinb) + (w + icb * W)) * 4 + l] = floatToHalf(c < Cin ? in[c * H * W + h * W + w] : 0.f); }
      inI.upload(runner, t.data()); }
    { std::vector<fp16_t> t((size_t)Cin * Coutb * 4, 0);  // kernel image [ic, ocb] = 4 oc
      for (int ocb = 0; ocb < Coutb; ++ocb) for (int ic = 0; ic < Cin; ++ic)
        for (int l = 0; l < 4; ++l) { int oc = ocb * 4 + l;
          t[(ocb * Cin + ic) * 4 + l] = floatToHalf(oc < Cout ? wt[oc * Cin + ic] : 0.f); }
      kI.upload(runner, t.data()); }

    // standalone storage-image pipeline
    VkDescriptorSetLayoutBinding b[3]{};
    for (int i = 0; i < 3; ++i) { b[i].binding = i; b[i].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
      b[i].descriptorCount = 1; b[i].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT; }
    VkDescriptorSetLayoutCreateInfo sl{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO};
    sl.flags = VK_DESCRIPTOR_SET_LAYOUT_CREATE_PUSH_DESCRIPTOR_BIT_KHR; sl.bindingCount = 3; sl.pBindings = b;
    VkDescriptorSetLayout dsl; vkCreateDescriptorSetLayout(ctx.device(), &sl, nullptr, &dsl);
    struct IPC { int W, H, Cin, Cout; } ipc{W, H, Cin, Cout};
    ...
    VkPipelineLayout playout; vkCreatePipelineLayout(ctx.device(), &pl, nullptr, &playout);
    const auto& spv = embeddedShaders().at("conv1x1_img");
    VkShaderModuleCreateInfo sm{VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO};
    sm.codeSize = spv.size() * 4; sm.pCode = spv.data();
    VkShaderModule mod; vkCreateShaderModule(ctx.device(), &sm, nullptr, &mod);
    ...
    cp.layout = playout;
    VkPipeline pipe; vkCreateComputePipelines(ctx.device(), VK_NULL_HANDLE, 1, &cp, nullptr, &pipe);
    ...
    auto rec = [&](VkCommandBuffer cmd) {
      ...
      for (int i = 0; i < 3; ++i) { w[i] = {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET}; w[i].dstBinding = i;
        w[i].descriptorCount = 1; w[i].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE; w[i].pImageInfo = &ii[i]; }
    ...
    VkCommandBuffer cmd = runner.allocate();
    runner.begin(cmd);
    for (int i = 0; i < iters; ++i) { rec(cmd); vk::computeBarrier(cmd); }
    runner.end(cmd);
    runner.submitAndWait(cmd);
    imgMs = runner.submitAndWait(cmd) / iters;
    vkFreeCommandBuffers(ctx.device(), runner.pool(), 1, &cmd);

    std::vector<fp16_t> t((size_t)W * Coutb * H * 4); outI.download(runner, t.data());
    for (int oc = 0; oc < Cout; ++oc) for (int h = 0; h < H; ++h) for (int w = 0; w < W; ++w)
      imgOut[oc * H * W + h * W + w] = halfToFloat(t[(h * (W * Coutb) + (w + (oc / 4) * W)) * 4 + oc % 4]);
    vkDestroyPipeline(ctx.device(), pipe, nullptr); vkDestroyShaderModule(ctx.device(), mod, nullptr);
    vkDestroyPipelineLayout(ctx.device(), playout, nullptr); vkDestroyDescriptorSetLayout(ctx.device(), dsl, nullptr);

    // ---- sampler2D / texelFetch path (true texture cache, like MNN) ----
    VkSampler samp;
    { VkSamplerCreateInfo si{VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO};
      si.magFilter = si.minFilter = VK_FILTER_NEAREST; si.addressModeU = si.addressModeV =
        si.addressModeW = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE; vkCreateSampler(ctx.device(), &si, nullptr, &samp); }
    VkDescriptorSetLayoutBinding tb[3]{};
    tb[0].binding = 0; tb[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    tb[1].binding = 1; tb[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    tb[2].binding = 2; tb[2].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
    for (int i = 0; i < 3; ++i) { tb[i].descriptorCount = 1; tb[i].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT; }
    VkDescriptorSetLayoutCreateInfo tsl{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO};
    tsl.flags = VK_DESCRIPTOR_SET_LAYOUT_CREATE_PUSH_DESCRIPTOR_BIT_KHR; tsl.bindingCount = 3; tsl.pBindings = tb;
    VkDescriptorSetLayout tdsl; vkCreateDescriptorSetLayout(ctx.device(), &tsl, nullptr, &tdsl);
    ...
    tpl.setLayoutCount = 1; tpl.pSetLayouts = &tdsl; tpl.pushConstantRangeCount = 1; tpl.pPushConstantRanges = &tpcr;
    VkPipelineLayout tlayout; vkCreatePipelineLayout(ctx.device(), &tpl, nullptr, &tlayout);
    const auto& tspv = embeddedShaders().at("conv1x1_tex");
    VkShaderModuleCreateInfo tsm{VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO};
    tsm.codeSize = tspv.size() * 4; tsm.pCode = tspv.data();
    VkShaderModule tmod; vkCreateShaderModule(ctx.device(), &tsm, nullptr, &tmod);
    ...
    tcp.stage.stage = VK_SHADER_STAGE_COMPUTE_BIT; tcp.stage.module = tmod; tcp.stage.pName = "main"; tcp.layout = tlayout;
    VkPipeline tpipe; vkCreateComputePipelines(ctx.device(), VK_NULL_HANDLE, 1, &tcp, nullptr, &tpipe);
    auto recT = [&](VkCommandBuffer cmd) {
      ...
      VkWriteDescriptorSet w[3]{};
      for (int i = 0; i < 3; ++i) { w[i] = {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET}; w[i].dstBinding = i;
        w[i].descriptorCount = 1; w[i].descriptorType = tb[i].descriptorType; w[i].pImageInfo = &ti[i]; }
    ...
    runner.begin(tc);
    for (int i = 0; i < iters; ++i) { recT(tc); vk::computeBarrier(tc); }
    runner.end(tc);
    runner.submitAndWait(tc);
    double texMs = runner.submitAndWait(tc) / iters;
    vkFreeCommandBuffers(ctx.device(), runner.pool(), 1, &tc);
    std::vector<fp16_t> tt((size_t)W * Coutb * H * 4); outI.download(runner, tt.data());
    std::vector<float> texOut(Cout * H * W);
    for (int oc = 0; oc < Cout; ++oc) for (int h = 0; h < H; ++h) for (int w = 0; w < W; ++w)
      texOut[oc * H * W + h * W + w] = halfToFloat(tt[(h * (W * Coutb) + (w + (oc / 4) * W)) * 4 + oc % 4]);
    printf("TEXFETCH(sampler2D): %.4f ms/iter cosine=%.5f => %.2fx vs SSBO %s\n", texMs, cosErr(texOut),
           ssboMs / texMs, texMs < ssboMs ? "FASTER" : "slower");
    vkDestroyPipeline(ctx.device(), tpipe, nullptr); vkDestroyShaderModule(ctx.device(), tmod, nullptr);
    vkDestroyPipelineLayout(ctx.device(), tlayout, nullptr); vkDestroyDescriptorSetLayout(ctx.device(), tdsl, nullptr);
    vkDestroySampler(ctx.device(), samp, nullptr);
  }

  printf("SSBO : %.4f ms/iter   cosine=%.5f\n", ssboMs, cosErr(ssboOut));
  if (imgMs >= 0) printf("IMAGE(storage/imageLoad): %.4f ms/iter cosine=%.5f => %.2fx vs SSBO %s\n", imgMs,
                         cosErr(imgOut), ssboMs / imgMs, imgMs < ssboMs ? "FASTER" : "slower");
  return 0;
}
#else
int main() { printf("built without Vulkan\n"); return 0; }
#endif
