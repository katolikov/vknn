...
int main(int argc, char **argv) {
    ...
    if (vk::Image::supported(ctx))
    {
        ...
        auto     rec    = [&](VkCommandBuffer cmd) {
            VkDescriptorImageInfo ii[3] = {{VK_NULL_HANDLE, inI.view(), VK_IMAGE_LAYOUT_GENERAL}, {VK_NULL_HANDLE, kI.view(), VK_IMAGE_LAYOUT_GENERAL}, {VK_NULL_HANDLE, outI.view(), VK_IMAGE_LAYOUT_GENERAL}};
        ...
        auto recT = [&](VkCommandBuffer cmd) {
            VkDescriptorImageInfo ti[3] = {{samp, inI.view(), VK_IMAGE_LAYOUT_GENERAL}, {samp, kI.view(), VK_IMAGE_LAYOUT_GENERAL}, {VK_NULL_HANDLE, outI.view(), VK_IMAGE_LAYOUT_GENERAL}};
            VkWriteDescriptorSet w[3] {};
            for (int i = 0; i < 3; ++i)
            {
                w[i]                 = {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};
