...
TEST(CpuOps, Conv1x1ReluReference) {
  ...
  wb.f32()[0] = 2; wb.f32()[1] = 0; wb.f32()[2] = 0; wb.f32()[3] = 2;
  g.initializers[w] = wb;
  TensorDesc bi; bi.name = "b"; bi.shape = {2}; bi.isInitializer = true;
  TensorId b = g.addTensor(bi);
  HostBuffer bb; bb.resizeElems(2, DType::kFloat32);
  bb.f32()[0] = -3; bb.f32()[1] = 0;
  g.initializers[b] = bb;
  TensorDesc yo; yo.name = "y"; yo.isOutput = true;
  TensorId y = g.addTensor(yo);

  Node conv; conv.type = OpType::kConv; conv.name = "conv";
  conv.inputs = {x, w, b}; conv.outputs = {y};
  g.nodes.push_back(conv);
  Node relu; relu.type = OpType::kRelu; relu.name = "relu";
  // relu reads y -> y2
  TensorDesc y2o; y2o.name = "y2"; TensorId y2 = g.addTensor(y2o);
  relu.inputs = {y}; relu.outputs = {y2};
  ...
  ASSERT_TRUE(sess);
  IOTensor in; in.name = "x"; in.shape = {1, 2, 1, 1}; in.dtype = DType::kFloat32;
  in.data.resize(2 * 4);
...
TEST(CpuOps, AddBroadcast) {
  ...
  TensorDesc bi; bi.name = "b"; bi.shape = {1, 3}; bi.isInitializer = true;
  TensorId b = g.addTensor(bi);
  HostBuffer bb; bb.resizeElems(3, DType::kFloat32);
  bb.f32()[0] = 10; bb.f32()[1] = 20; bb.f32()[2] = 30; g.initializers[b] = bb;
  TensorDesc co; co.name = "c"; co.isOutput = true; TensorId c = g.addTensor(co);
  Node add; add.type = OpType::kAdd; add.name = "add"; add.inputs = {a, b}; add.outputs = {c};
  g.nodes.push_back(add); g.outputs = {c};
  Config cfg; cfg.backend = BackendKind::kCpu;
  auto sess = Session::create(std::move(g), cfg);
