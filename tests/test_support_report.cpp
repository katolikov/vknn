...
TEST(SupportReport, CastFromInt64TargetGate) {
    ...
    // UINT8 (saturate) to match the CPU Cast op + readback narrowing bit-for-bit. BOOL is a truth test
    // (any nonzero value stores 1). INT16/UINT16 and the 32/64-bit unsigned targets keep the CPU op.
    // `to` is the ONNX TensorProto dtype.
    ...
    castNode(g, "cast_i64_to_bool", 9, DType::Int64);  // BOOL   -> GPU (truth test)
    ...
    EXPECT_EQ(rows[6].backend, "vulkan"); // BOOL (truth test)
