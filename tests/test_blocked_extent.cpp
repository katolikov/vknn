...
TEST(BlockedExtent, FlatTensorConvertsItsLogicalCount) {
    EXPECT_EQ(storedElemCount({1, 2, 144, 96}, /*gpuFlat=*/true), 1 * 2 * 144 * 96);
    EXPECT_EQ(storedElemCount({1, 3, 8, 8}, /*gpuFlat=*/true), 1 * 3 * 8 * 8);
    EXPECT_EQ(storedElemCount({7}, /*gpuFlat=*/true), 7);
}

TEST(BlockedExtent, BlockedTensorConvertsThePaddedFootprint) {
    ...
    EXPECT_EQ(storedElemCount({1, 2, 144, 96}, /*gpuFlat=*/false), 1 * 4 * 144 * 96);
    EXPECT_EQ(storedElemCount({1, 1, 8, 8}, /*gpuFlat=*/false), 1 * 4 * 8 * 8);
    EXPECT_EQ(storedElemCount({1, 3, 8, 8}, /*gpuFlat=*/false), 1 * 4 * 8 * 8);
    EXPECT_EQ(storedElemCount({2, 5, 3, 3}, /*gpuFlat=*/false), 2 * 8 * 3 * 3);
}

TEST(BlockedExtent, BlockedAndFlatAgreeOnlyOnFullBlocks) {
    for (int64_t c = 1; c <= 16; ++c)
    {
        const Shape   s {1, c, 5, 7};
        const int64_t flat = storedElemCount(s, true), blocked = storedElemCount(s, false);
...
TEST(BlockedExtent, RankBelowFourStillCountsItsBlocks) {
    // NCHW::from right-aligns, so a rank-2 [N,C] tensor is C channels at 1x1 and still blocks.
    EXPECT_EQ(storedElemCount({4, 6}, /*gpuFlat=*/false), 4 * 8 * 1 * 1);
    EXPECT_EQ(storedElemCount({4, 6}, /*gpuFlat=*/true), 4 * 6);
}
