...
namespace vknn {
    ...
    enum class Hint {
        ...
        FusedAttention  = 7, ///< Fuse the M=1 decode-attention chain into one FusedAttention kernel at load (On / Off, default On).
    };

    /// Every kernel/pass selection value, set uniformly via setHint(Hint, Mode). The value sets by
    /// Hint: Auto/On/Off serve Winograd, FlatLayout, GpuIslandFold, MatMulViewFold and FusedAttention;
