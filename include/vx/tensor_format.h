// vxrt — tensor layout formats and helpers. Canonical IR layout is NCHW.
#pragma once
#include <cstdint>
#include "vx/common.h"
...
inline const char* formatStr(TensorFormat f) {
  switch (f) {
    case TensorFormat::kNCHW: return "NCHW";
    case TensorFormat::kNHWC: return "NHWC";
    case TensorFormat::kNC4HW4: return "NC4HW4";
    default: return "?";
  }
}
...
struct NCHW {
  ...
  static NCHW from(const Shape& s) {
    NCHW r;
    if (s.size() == 4) { r.n = s[0]; r.c = s[1]; r.h = s[2]; r.w = s[3]; }
    else if (s.size() == 2) { r.n = s[0]; r.c = s[1]; r.h = 1; r.w = 1; }
    else if (s.size() == 1) { r.c = s[0]; }
    return r;
  }
...
inline int64_t cBlocks(int64_t c) { return (c + 3) / 4; }
