...
#define VKNN_REGISTER_CPU_OP(OPTYPE, CLASS)                            \
    ...
    namespace cpu {
        ...
        /// fp64 counterpart of `allocOut`, for ops that compute in double. Sizes `rt` to `shape` at
        /// 8 bytes/elem, labels it Float64, and returns a typed double pointer.
        double *allocOutF64(RtTensor &rt, const Shape &shape);
        /// Size `rt` to `shape` at `dt` and return the raw element pointer, labeling the tensor `dt`.
        /// For dtype-preserving movement ops that relocate elements of any width without converting them;
        /// the caller casts the pointer to the element type matching `dt`.
