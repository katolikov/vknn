...
namespace vknn { namespace vk {
    ...
    struct VulkanCaps {
        ...
        // Core VkPhysicalDeviceFeatures::shaderFloat64 — 64-bit doubles in shader code. fp64 ops run on
        // the CPU today (mobile drivers expose no usable double precision); this is reported for
        // diagnostics and is the gate a future fp64 GPU kernel would key on. No such kernel ships yet.
