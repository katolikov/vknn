// Flat (row-major) Equal on the GPU: out = (a == b) ? 1 : 0, with N-D broadcasting up to
// flat::kMaxRank. Always runs on the flat path (gpuFlatNode returns true). Constant operands are
// uploaded flat in prepare(), exactly like flat::Binary. Push-constant block byte-matches
// shaders/equal.comp.
#include "flat_ops.h"
    ...
    namespace {
        ...
        struct EqualVk: VulkanOp {
            ...
            void prepare(const Node &node, VkOpEnv &env) override {
                ...
                pipe = env.pipeline(shader("equal", env.useFp16), 3, sizeof(PC), std::vector<uint32_t> {});
            }
            ...
            void record(VkCommandBuffer cmd, const Node &node, VkOpEnv &env) override {
                ...
                // One flat invocation per output element; equal.comp is local_size_x=256 == flat::kFlatLocalSize.
