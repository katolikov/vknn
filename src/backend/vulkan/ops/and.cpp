// Flat (row-major) boolean AND on the GPU: out = (a != 0 && b != 0) ? 1 : 0, with N-D broadcasting
// at any rank. Always runs on the flat path (gpuFlatNode returns true). Constant operands are
// uploaded flat in prepare(), exactly like Equal. The per-axis outDim/aStride/bStride geometry rides
// a content-deduped SSBO (flat::uploadFlatGeom) bound at binding 3; the push constant carries only
// rank/total. Layout byte-matches shaders/and.comp.
#include "flat_ops.h"
...
namespace vknn {
    namespace {
        ...
        struct AndVk: VulkanOp {
            ...
            void prepare(const Node &node, VkOpEnv &env) override {
                ...
                pipe = env.pipeline(shader("and", env.useFp16), 4, sizeof(PC), std::vector<uint32_t> {env.flatLocalSize});
            }
            ...
            void record(VkCommandBuffer cmd, const Node &node, VkOpEnv &env) override {
                ...
                // One flat invocation per output element; and.comp is local_size_x=256 == flat::kFlatLocalSize.
    ...
    VKNN_REGISTER_VK_OP(OpType::And, AndVk);
} // namespace vknn
