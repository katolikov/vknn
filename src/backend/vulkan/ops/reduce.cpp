...
namespace vknn {
    namespace {
        ...
        struct ReduceOp: VulkanOp {
            ...
            ReducePCFlat                         pc {};
            std::shared_ptr<vk::Buffer>          geom; // inDim/inStride/reduce, deduped SSBO (binding 2, before the epilogue)
            PwEpi                                epi;
            void                                 prepare(const Node &node, VkOpEnv &env) override {
                ...
                // _epi shader variant and epi.extraBufs() adds its operand bindings on top of the three
                // base buffers (input, output, geometry).
            ...
            void record(VkCommandBuffer cmd, const Node &node, VkOpEnv &env) override {
                // Binding order matches the shader: input at 0, output at 1, geometry at 2, then any
                // fused-epilogue operands appended by epi.append (in sync with the extraBufs() count above).
