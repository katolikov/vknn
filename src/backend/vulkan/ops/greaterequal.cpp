// Flat (row-major) GreaterOrEqual on the GPU: out = (a >= b) ? 1 : 0, with N-D broadcasting at any rank.
...
namespace vknn {
    namespace {
        ...
        struct GreaterEqualVk: VulkanOp {
            struct PC {
                int rank, total;
            } pc {};
            ...
            void prepare(const Node &node, VkOpEnv &env) override {
                ...
                pipe = env.pipeline(shader("greaterequal", env.useFp16), 4, sizeof(PC), std::vector<uint32_t> {env.flatLocalSize});
            }
    ...
    VKNN_REGISTER_VK_OP(OpType::GreaterEqual, GreaterEqualVk);
} // namespace vknn
