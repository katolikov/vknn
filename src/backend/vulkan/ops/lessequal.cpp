// Flat (row-major) LessOrEqual on the GPU: out = (a <= b) ? 1 : 0, with N-D broadcasting at any rank.
...
// byte-matches shaders/lessequal.comp.
#include "flat_ops.h"
...
namespace vknn {
    namespace {
        ...
        struct LessEqualVk: VulkanOp {
            ...
            void prepare(const Node &node, VkOpEnv &env) override {
                ...
                pipe = env.pipeline(shader("lessequal", env.useFp16), 4, sizeof(PC), std::vector<uint32_t> {env.flatLocalSize});
            }
            ...
            void record(VkCommandBuffer cmd, const Node &node, VkOpEnv &env) override {
                ...
                // One flat invocation per output element; lessequal.comp is local_size_x=256 == flat::kFlatLocalSize.
    ...
    VKNN_REGISTER_VK_OP(OpType::LessEqual, LessEqualVk);
} // namespace vknn
