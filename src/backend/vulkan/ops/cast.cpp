        ...
        struct CastOp: VulkanOp {
            ...
            void prepare(const Node &node, VkOpEnv &env) override {
                ...
                if (truncate)
                {
                    ...
                    // at the device-resolved width (spec 0 = env.flatLocalSize, matching groups() below).
                    pipe = env.pipeline(shader("cast", env.useFp16), 2, sizeof(PC), std::vector<uint32_t> {env.flatLocalSize});
                }
            }
            void record(VkCommandBuffer cmd, const Node &node, VkOpEnv &env) override {
                ...
                pipe->dispatch(cmd, {src->handle(), dst->handle()}, &pc, sizeof(pc), groups(pc.total, env.flatLocalSize));
            }
        };
