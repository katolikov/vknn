...
namespace vknn {
    ...
    namespace {
        ...
        constexpr unsigned long kDmaHeapIoctlAlloc = (3UL << 30) | (sizeof(dma_heap_allocation_data) << 16) | ('H' << 8) | 0x0;
    } // namespace
    ...
    std::unique_ptr<IonBuffer> IonBuffer::alloc(size_t bytes, const std::string &heap) {
        ...
        VKNN_INFO << "ION: allocated " << bytes << " bytes from /dev/dma_heap/" << heap << " -> fd " << b->fd_ << (b->map_ ? " (mapped)" : " (unmapped)");
        return b;
    }
    ...
    std::unique_ptr<IonBuffer> IonBuffer::wrapFd(int fd, size_t bytes, bool takeOwnership) {
        ...
        VKNN_INFO << "ION: wrapped fd " << fd << " (" << bytes << " bytes, ownership=" << (takeOwnership ? "engine" : "caller") << ")";
        return b;
    }
