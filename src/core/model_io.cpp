...
struct Writer {
  FILE* f;
  template <typename T> void pod(const T& v) { fwrite(&v, sizeof(T), 1, f); }
  void u32(uint32_t v) { pod(v); }
  void i64(int64_t v) { pod(v); }
  void f32(float v) { pod(v); }
  void str(const std::string& s) { u32((uint32_t)s.size()); if (!s.empty()) fwrite(s.data(), 1, s.size(), f); }
  template <typename T> void vec(const std::vector<T>& v) { u32((uint32_t)v.size()); if (!v.empty()) fwrite(v.data(), sizeof(T), v.size(), f); }
};
struct Reader {
  FILE* f;
  bool ok = true;
  template <typename T> T pod() { T v{}; ok = ok && fread(&v, sizeof(T), 1, f) == 1; return v; }
  uint32_t u32() { return pod<uint32_t>(); }
  int64_t i64() { return pod<int64_t>(); }
  float f32() { return pod<float>(); }
  std::string str() { uint32_t n = u32(); std::string s(n, 0); if (n) ok = ok && fread(&s[0], 1, n, f) == n; return s; }
  template <typename T> std::vector<T> vec() { uint32_t n = u32(); std::vector<T> v(n); if (n) ok = ok && fread(v.data(), sizeof(T), n, f) == n; return v; }
};
...
bool saveGraphBin(const Graph& g, const std::string& path) {
  ...
  if (!f) { VX_WARN << "saveGraph: cannot write " << path; return false; }
  Writer w{f};
  w.u32(kMagic);
  // tensors
  ...
  for (const auto& n : g.nodes) {
    ...
    for (const auto& kv : n.attr.map) { w.str(kv.first); writeAttr(w, kv.second); }
  }
  w.vec(g.inputs);
  w.vec(g.outputs);
  // initializers
  w.u32((uint32_t)g.initializers.size());
  for (const auto& kv : g.initializers) { w.i64(kv.first); w.vec(kv.second.bytes); }
  fclose(f);
...
bool loadGraphBin(Graph& g, const std::string& path) {
  ...
  if (!f) return false;
  Reader r{f};
  if (r.u32() != kMagic) { fclose(f); VX_WARN << "loadGraph: bad magic in " << path; return false; }
  g = Graph{};
  uint32_t nt = r.u32();
  g.tensors.resize(nt);
  for (uint32_t i = 0; i < nt; ++i) {
    ...
    t.isInput = flags & 1; t.isOutput = flags & 2; t.isInitializer = flags & 4;
    if (!t.name.empty()) g.tensorByName[t.name] = (TensorId)i;
  }
  uint32_t nn = r.u32();
  g.nodes.resize(nn);
  for (uint32_t i = 0; i < nn; ++i) {
    ...
    uint32_t na = r.u32();
    for (uint32_t a = 0; a < na; ++a) { std::string k = r.str(); n.attr.map[k] = readAttr(r); }
  }
  ...
  fclose(f);
  if (!r.ok) { VX_WARN << "loadGraph: truncated " << path; return false; }
  return true;
}
