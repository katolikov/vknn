 ...
 public:
  ...
  void skip(uint32_t wire) {
    switch (wire) {
      ...
      case 1: p_ += 8; break;
      case 5: p_ += 4; break;
      case 2: { uint64_t l = varint(); p_ += l; break; }
      default: break;
    }
  }
...
static TensorProto parseTensor(Reader r) {
  ...
  while (r.tag(f, w)) {
    switch (f) {
      case 1:  // dims
        if (w == 2) { Reader s = r.sub(); while (!s.eof()) t.dims.push_back((int64_t)s.varint()); }
        else t.dims.push_back((int64_t)r.varint());
        break;
      case 2: t.dataType = (int32_t)r.varint(); break;
      case 4:  // float_data (packed or single)
        if (w == 2) { Reader s = r.sub(); while (!s.eof()) { uint32_t b = s.fixed32(); float fv; std::memcpy(&fv,&b,4); t.floatData.push_back(fv); } }
        else { uint32_t b = r.fixed32(); float fv; std::memcpy(&fv,&b,4); t.floatData.push_back(fv); }
        break;
      case 7:  // int64_data
        if (w == 2) { Reader s = r.sub(); while (!s.eof()) t.int64Data.push_back((int64_t)s.varint()); }
        else t.int64Data.push_back((int64_t)r.varint());
        break;
      case 8: t.name = r.str(); break;
...
static void fillHostFloat(const TensorProto& t, HostBuffer& hb, int64_t elems) {
  ...
  if (!t.raw.empty()) {
    ...
    for (int64_t i = 0; i < elems && i < (int64_t)t.int64Data.size(); ++i) dst[i] = (float)t.int64Data[i];
  }
}
...
static void parseAttr(Reader r, Node& node) {
  ...
  while (r.tag(f, w)) {
    switch (f) {
      ...
      case 2: { uint32_t b = r.fixed32(); std::memcpy(&a.f, &b, 4); a.kind = Attr::kFloat; break; }
      case 3: a.i = (int64_t)r.varint(); a.kind = Attr::kInt; break;
      case 4: { auto b = r.bytes(); a.str.assign((const char*)b.data(), b.size()); a.kind = Attr::kString; break; }
      case 7: { Reader s = r.sub(); while (!s.eof()) { uint32_t b = s.fixed32(); float fv; std::memcpy(&fv,&b,4); a.floats.push_back(fv); } a.kind = Attr::kFloats; break; }
      case 8: { if (w==2){ Reader s = r.sub(); while (!s.eof()) a.ints.push_back((int64_t)s.varint()); } else a.ints.push_back((int64_t)r.varint()); a.kind = Attr::kInts; break; }
      case 5: { tp = parseTensor(r.sub()); hasTp = true; break; }
      default: r.skip(w); break;
    }
  }
  if (hasTp) {
    ...
    a.kind = Attr::kFloats;
    int64_t n = 1; for (auto d : tp.dims) n *= d; if (tp.dims.empty()) n = std::max<int64_t>(1,(int64_t)std::max(tp.floatData.size(), tp.int64Data.size()));
    if (!tp.int64Data.empty() || tp.dataType == 7) {
      a.kind = Attr::kInts;
      if (!tp.int64Data.empty()) a.ints = tp.int64Data;
      else if (!tp.raw.empty()) { const int64_t* s=(const int64_t*)tp.raw.data(); for(int64_t i=0;i<n;++i) a.ints.push_back(s[i]); }
    } else {
      if (!tp.floatData.empty()) a.floats = tp.floatData;
      else if (!tp.raw.empty()) { const float* s=(const float*)tp.raw.data(); for(int64_t i=0;i<n;++i) a.floats.push_back(s[i]); }
    }
  }
    ...
    if (f == 1) name = r.str();
      ...
      while (tp.tag(f2, w2)) {
        if (f2 == 1) {  // tensor_type
          ...
          uint32_t f3, w3;
          while (tt.tag(f3, w3)) {
            if (f3 == 1) elem = (int32_t)tt.varint();
            else if (f3 == 2) {  // shape
              Reader sh = tt.sub();
              uint32_t f4, w4;
              while (sh.tag(f4, w4)) {
                if (f4 == 1) {  // dim
                  ...
                  int64_t val = -1;
                  while (dim.tag(f5, w5)) {
                    if (f5 == 1) val = (int64_t)dim.varint();        // dim_value
                    else if (f5 == 2) { dim.str(); val = -1; }       // dim_param (dynamic)
                    else dim.skip(w5);
                  }
                  shape.push_back(val);
                } else sh.skip(w4);
...
// ----------------------------- NodeProto -----------------------------
// fields: 1=input(string repeated), 2=output(string repeated), 3=name, 4=op_type, 5=attribute, 7=domain
static void parseNode(Reader r, Graph& g, Node& node) {
  ...
  while (r.tag(f, w)) {
    switch (f) {
      case 1: ins.push_back(r.str()); break;
      case 2: outs.push_back(r.str()); break;
      case 3: node.name = r.str(); break;
      case 4: opType = r.str(); break;
      case 5: parseAttr(r.sub(), node); break;
      default: r.skip(w); break;
    }
  }
...
static void parseGraph(Reader r, Graph& g) {
  uint32_t f, w;
  struct PendingInit { std::string name; TensorProto tp; };
  std::vector<PendingInit> inits;
  std::vector<Node> nodes;
  while (r.tag(f, w)) {
    switch (f) {
      case 1: { Node n; parseNode(r.sub(), g, n); nodes.push_back(std::move(n)); break; }
      case 5: { TensorProto tp = parseTensor(r.sub()); std::string nm = tp.name; inits.push_back({nm, std::move(tp)}); break; }
      case 11: { std::string nm; Shape sh; int32_t el = 1; parseValueInfo(r.sub(), nm, sh, el); TensorId id = g.findOrAdd(nm); g.desc(id).shape = sh; g.desc(id).isInput = true; g.inputs.push_back(id); break; }
      case 12: { std::string nm; Shape sh; int32_t el = 1; parseValueInfo(r.sub(), nm, sh, el); TensorId id = g.findOrAdd(nm); g.desc(id).shape = sh; g.desc(id).isOutput = true; g.outputs.push_back(id); break; }
      case 13: { std::string nm; Shape sh; int32_t el = 1; parseValueInfo(r.sub(), nm, sh, el); if (!nm.empty()) { TensorId id = g.findOrAdd(nm); if (g.desc(id).shape.empty()) g.desc(id).shape = sh; } break; }
      default: r.skip(w); break;
    }
  }
  ...
  for (auto& pi : inits) {
    ...
    d.shape = pi.tp.dims;
    int64_t n = 1; for (auto x : pi.tp.dims) n *= x; if (pi.tp.dims.empty()) n = 1;
    HostBuffer hb;
    if (pi.tp.dataType == 7) { d.dtype = DType::kInt64; fillHostI64(pi.tp, hb, n); }
    else { d.dtype = DType::kFloat32; fillHostFloat(pi.tp, hb, n); }
...
Graph importOnnx(const std::string& path) {
  ...
  while (r.tag(fld, wire)) {
    if (fld == 7 && wire == 2) { onnx::parseGraph(r.sub(), g); foundGraph = true; }
    else r.skip(wire);
  }
