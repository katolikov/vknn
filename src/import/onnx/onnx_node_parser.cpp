...
namespace vknn { namespace onnx {
    ...
    void NodeParser::parseValueInfo(Reader r, std::string &name, Shape &shape, int32_t &elem, std::vector<std::string> *dimParams) {
        uint32_t f, w;
        while (r.tag(f, w))
        {
            if (f == kValueInfoName)
    ...
    void NodeParser::parseNode(Reader r, Node &node, std::vector<std::string> &ins, std::vector<std::string> &outs, const std::string &baseDir, std::map<std::string, std::vector<uint8_t>> *extCache) {
        ...
        } else if (opTypeIsQuantized(node.type))
        {
            ...
            VKNN_WARN << "quantized ONNX op '" << opType << "' (node " << node.name << ") - runs only via the dequantize lowering";
        }
    }
