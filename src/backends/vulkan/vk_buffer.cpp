...
Buffer::Buffer(VulkanContext& ctx, size_t bytes, MemPref pref, VkBufferUsageFlags extraUsage)
...
Buffer* Buffer::importDmaBufFd(VulkanContext& ctx, int fd, size_t bytes,
  ...
  try {
    ...
    auto pfnGetFdProps = (PFN_vkGetMemoryFdPropertiesKHR)vkGetDeviceProcAddr(
