...
struct WhereVk : VulkanOp {
  struct PC {
    int rank, total;
    int outDim[flat::kMaxRank], cStride[flat::kMaxRank], xStride[flat::kMaxRank],
        yStride[flat::kMaxRank];
  } pc{};
  ...
  void prepare(const Node& node, VkOpEnv& env) override {
    ...
    pipe = std::make_unique<vk::ComputePipeline>(*env.ctx, shader("where_select", env.useFp16), 4,
  ...
  void record(VkCommandBuffer cmd, const Node& node, VkOpEnv& env) override {
    ...
    pipe->dispatch(cmd,
                   {buf(0)->handle(), buf(1)->handle(), buf(2)->handle(),
