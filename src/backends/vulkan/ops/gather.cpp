// Flat row-major GPU Gather (one op per file). Honors `axis` and a scalar-or-N-D index. The index is
// passed to the kernel as a float buffer so ONE kernel serves both cases: a constant index (attention
// Q/K/V split, uploaded here as float) and a runtime float index activation (RoPE position lookup,
// read straight from its device buffer). Mirrors GatherCpu's shape/scatter math.
#include <algorithm>
#include <vector>
...
struct GatherOp : VulkanOp {
  struct PC { int total, outer, axisSize, inner, nIdx; } pc{};
  ...
  void prepare(const Node& node, VkOpEnv& env) override {
    ...
    int64_t outer = 1;
    for (int k = 0; k < axis; ++k) outer *= d[k];
    int64_t inner = 1;
    for (int k = axis + 1; k < rank; ++k) inner *= d[k];
