...
struct ScatterNDOp : VulkanOp {
  ...
  void prepare(const Node& node, VkOpEnv& env) override {
    ...
    int64_t rows = 1;
    for (size_t i = 0; i + 1 < is.size(); ++i) rows *= is[i];
    ...
    // Index: a constant initializer is uploaded as float; a runtime float index activation (e.g. the
    // decoder/camera ScatterNDs) is read straight from its device buffer. Both feed the kernel's
    // float IDX binding (the kernel truncates to int), so one kernel serves both.
    ...
    if (g.isInitializer(iid)) {
      ...
      for (int64_t i = 0; i < nIdx; ++i) {
        if (idt == DType::kInt64) idxf[(size_t)i] = (float)ib.i64()[i];
        else if (idt == DType::kFloat16)
          idxf[(size_t)i] = halfToFloat(reinterpret_cast<const fp16_t*>(ib.bytes.data())[i]);
        else idxf[(size_t)i] = ib.f32()[i];
      }
      idxBuf = upload(*env.ctx, idxf, env.useFp16);
    }

    copyPipe = std::make_unique<vk::ComputePipeline>(*env.ctx, shader("scatternd_copy", env.useFp16),
                                                     2, sizeof(CopyPC), std::vector<uint32_t>{},
                                                     env.cache->handle());
    scatterPipe = std::make_unique<vk::ComputePipeline>(*env.ctx, shader("scatternd", env.useFp16),
                                                        3, sizeof(ScatterPC),
  ...
  void record(VkCommandBuffer cmd, const Node& node, VkOpEnv& env) override {
    ...
    // The framework only barriers BETWEEN nodes (read-after-write across ops); two dispatches inside
    // one record() are NOT auto-barriered. Pass 2 scatters into the SAME `out` buffer pass 1 wrote,
    // so without this compute->compute barrier the dispatches can overlap and read stale data.
    vk::computeBarrier(cmd);
    // Pass 2: scatter updates into out at the index rows.
    if (pc.total > 0)
      scatterPipe->dispatch(cmd, {updates->handle(), idx->handle(), out->handle()}, &pc,
