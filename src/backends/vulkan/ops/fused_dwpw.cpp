...
struct FusedDwPwOp : VulkanOp {
  ...
  void prepare(const Node& node, VkOpEnv& env) override {
    ...
    pc = {(int)x.n,    (int)E,      (int)x.h,    (int)x.w,        (int)Cout,          (int)y.h,
          (int)y.w,    (int)KH,     (int)KW,     (int)st[0],      (int)st[1],         (int)pad[0],
          (int)pad[1], (int)dil[0], (int)dil[1], (int)node.subOp, (int)node.fusedAct, node.actLo,
          node.actHi};
    ...
    pww = uploadCached(env, node.name + "#pww", [&] {  // [Cout][Eb][4]
      ...
      for (int64_t oc = 0; oc < Cout; ++oc)
        for (int64_t ic = 0; ic < E; ++ic) {
          ...
          wp[((oc * Eb + icb) * 4) + l] = pwsrc[oc * E + ic];
        }
      return wp;
    });
    ...
    pipe = std::make_unique<vk::ComputePipeline>(
        *env.ctx, shader("fused_dwpw", env.useFp16), hasRes ? 7 : 6, sizeof(DwPwPC),
        std::vector<uint32_t>{(uint32_t)(hasRes ? 1 : 0)}, env.cache->handle());
  }
  void record(VkCommandBuffer cmd, const Node& node, VkOpEnv& env) override {
    vk::Buffer* exp = env.devBuf(node.inputs[0]);
    vk::Buffer* dst = env.devBuf(node.outputs[0]);
    std::vector<VkBuffer> b = {exp->handle(), dww->handle(), dwb->handle(),
                               pww->handle(), pwb->handle(), dst->handle()};
    if (hasRes)
