// Batched N-D MatMul on the FLAT row-major GPU path: out[...,m,n] = sum_k A[...,m,k]*B[...,k,n] with
// NumPy broadcasting on the batch dims. One thread per output element walks K (see shaders/matmul.comp).
// Either operand may be an activation or a constant initializer (e.g. a Linear weight); an initializer
// is uploaded flat in prepare(). Mirrors the CPU oracle's broadcast/stride math byte-for-byte.
#include <algorithm>
#include <vector>
#include "flat_ops.h"
...
struct MatMulOp : VulkanOp {
  ...
  void prepare(const Node& node, VkOpEnv& env) override {
    ...
    // Promote 1-D operands (A[K]->[1,K], B[K]->[K,1]) to find M/N/K; the output rank already had the
    // promoted dim stripped by inferShapes, so we work the strides against `out` directly below.
    ...
    if (bWas1D) nAxis = -1;               // N axis was stripped
    // batch dims occupy output indices [0, firstMatAxis)
    int firstMatAxis = rank;
    if (mAxis >= 0) firstMatAxis = std::min(firstMatAxis, mAxis);
    if (nAxis >= 0) firstMatAxis = std::min(firstMatAxis, nAxis);
    ...
    // Matrix-axis strides: A depends on m (row stride K) not n; B depends on n (col stride 1) not m.
    if (mAxis >= 0) { pc.aStride[mAxis] = (int)K; pc.bStride[mAxis] = 0; }
    if (nAxis >= 0) { pc.aStride[nAxis] = 0;      pc.bStride[nAxis] = 1; }
    ...
    pipe = std::make_unique<vk::ComputePipeline>(*env.ctx, shader("matmul", env.useFp16), 3,
                                                 sizeof(MatMulPC), std::vector<uint32_t>{},
                                                 env.cache->handle());
  }
