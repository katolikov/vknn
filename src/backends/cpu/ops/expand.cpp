...
// the target is the broadcast of X.shape and `shape`, so output dims can exceed both). dtype-agnostic.
// out[i] = in[ sum_k (outCoord_k % inDim_k) * inStride_k ] with input right-aligned into the output.
#include <algorithm>
...
struct ExpandCpu : CpuOp {
  void run(const Node& node, ExecContext& ctx) override {
    ...
    for (int k = 0; k < (int)in.size(); ++k) pin[rank - (int)in.size() + k] = in[k];
    ...
    int64_t acc = 1;
    for (int k = rank - 1; k >= 0; --k) { inStride[k] = (pin[k] == 1) ? 0 : acc; acc *= pin[k]; }
    for (int k = rank - 2; k >= 0; --k) outStride[k] = outStride[k + 1] * out[k + 1];
    ...
    for (int64_t oi = 0; oi < elems; ++oi) {
      ...
      for (int k = 0; k < rank; ++k) { int64_t c = rem / outStride[k]; rem %= outStride[k]; inf += c * inStride[k]; }
