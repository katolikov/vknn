// Tile (ONNX): repeat X `repeats[k]` times along each axis (repeats is an int64 input, len == rank).
// out.shape[k] = X.shape[k] * repeats[k]; out[i] = in[ sum_k (outCoord_k % inDim_k) * inStride_k ].
// dtype-agnostic.
#include <algorithm>
...
struct TileCpu : CpuOp {
  void run(const Node& node, ExecContext& ctx) override {
    ...
    for (int k = rank - 2; k >= 0; --k) { inStride[k] = inStride[k + 1] * in[k + 1]; outStride[k] = outStride[k + 1] * out[k + 1]; }
    ...
    for (int64_t oi = 0; oi < elems; ++oi) {
      ...
      for (int k = 0; k < rank; ++k) { int64_t c = rem / outStride[k]; rem %= outStride[k]; inf += (c % in[k]) * inStride[k]; }
