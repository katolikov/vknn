...
struct ConstantCpu : CpuOp {
  void run(const Node& node, ExecContext& ctx) override {
    ...
    // multi-dim constant (e.g. a (1,2,8400) anchor grid against (1,2,8400) features -> bogus shape).
    auto shapeOf = [&](size_t n) -> Shape {
      if (it != node.attr.map.end() && !it->second.shape.empty()) return it->second.shape;
      return {(int64_t)n};
    };
