...
// the value operands. Output dtype follows X/Y: the dynamic-shape subgraph runs Where on INT64 shape
// vectors (e.g. Where(Equal(dim,-1), input_shape, target)), so reading/writing those as fp32 would
// reinterpret the int bytes as garbage — the value operands are read in their native dtype.
#include <algorithm>
