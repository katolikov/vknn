// Pad (constant/edge/reflect), generic N-D. pads = [begin..., end...] (2*rank) from attr or input[1];
// constant value from attr "value" or input[2].
#include <algorithm>
...
struct PadCpu : CpuOp {
  ...
  void run(const Node& node, ExecContext& ctx) override {
    ...
    if (node.inputs.size() > 2 && node.inputs[2] != kNoTensor) cval = ctx.t(node.inputs[2]).host.f32()[0];
    ...
    auto reflect = [](int64_t i, int64_t n) { if (n == 1) return (int64_t)0; int64_t p = 2 * (n - 1); i = ((i % p) + p) % p; return i < n ? i : p - i; };
    for (int64_t oi = 0; oi < elems; ++oi) {
      int64_t rem = oi, inf = 0; bool oob = false;
      for (int i = 0; i < rank; ++i) {
        int64_t oc = rem / outStride[i]; rem %= outStride[i];
        int64_t ic = oc - (pads.empty() ? 0 : pads[i]);
        if (ic < 0 || ic >= X.shape[i]) {
          if (mode == "edge") ic = std::min<int64_t>(std::max<int64_t>(ic, 0), X.shape[i] - 1);
          else if (mode == "reflect") ic = reflect(ic, X.shape[i]);
          else { oob = true; break; }
        }
        inf += ic * inStride[i];
      }
